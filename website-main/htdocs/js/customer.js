...
async function loadCustomerDashboard() {
  ...
  try {
    // Check if user is logged in with timeout (increased for slow connections)
    ...
    // Get user with timeout and fallback (increased for slow connections)
    ...
    // 1. Warning panel with timeout and caching (increased for slow connections)
    loadingPromises.push(
      Promise.race([
        renderWarningPanel(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Warning panel timeout')), 8000))
      ]).catch(error => console.warn('[Dashboard] Warning panel failed:', error.message))
    );
    
    // 2. Auto-cancel check with timeout (increased for slow connections)
    if (typeof checkAndCancelPendingBookings === 'function') {
      loadingPromises.push(
        Promise.race([
          checkAndCancelPendingBookings(),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Auto-cancel timeout')), 8000))
        ]).then(() => {
          // Clear cache after auto-cancel to ensure fresh data
    ...
    // 3. User bookings with caching (increased for slow connections)
    loadingPromises.push(
      Promise.race([
        loadUserBookingsWithCache(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Bookings timeout')), 15000))
      ]).then(() => {
...
function switchCustomerView(view) {
  ...
  // Load data if needed
  if (view === 'bookings') {
    // On mobile, show team calendar; on desktop, show booking slots
...
// Mobile view switching with bottom navigation
// Reuses existing switchCustomerView function, just adds mobile-specific logic
function switchCustomerViewMobile(view) {
  // Close more menu if open
  closeCustomerMoreMenu();
  
  // Switch the view using existing function
  if (typeof switchCustomerView === 'function') {
    switchCustomerView(view);
  }
  
  // Update bottom nav active state
  updateCustomerBottomNavActive(view);
}

/**
 * Update bottom nav active state
 */
function updateCustomerBottomNavActive(view) {
  ...
  const moreViews = ['profile', 'account'];
  if (moreViews.includes(view)) {
    const moreBtn = document.getElementById('customerMoreMenuBtn');
 ...
 */
function toggleCustomerMoreMenu() {
  const moreMenu = document.getElementById('customerMoreMenu');
  if (moreMenu) {
 ...
 * Close more menu
 */
function closeCustomerMoreMenu() {
  const moreMenu = document.getElementById('customerMoreMenu');
  if (moreMenu) {
...
function getFilteredCustomerBookings(bookings = customerBookingsCache) {
  ...
  switch (filter) {
    ...
    case 'all':
    default:
      // Default 'all' filter: exclude cancelled bookings to keep recent bookings clean
      result = bookings.filter(b => !cancelledStatuses.includes(b.status));
      break;
  }
...
function getBookingStatusInfo(status) {
  const statusMap = {
    ...
    'inprogress': { label: 'In Progress', bgColor: '#cfe2ff', textColor: '#084298' },
    'completed': { label: 'Completed', bgColor: '#e2e3e5', textColor: '#383d41' },
    'cancelled': { label: 'Cancelled', bgColor: '#f8d7da', textColor: '#842029' }
  };
...
async function loadCalendarViewStats() {
  try {
    ...
    // Determine which container to populate based on screen size
    const isMobile = window.innerWidth <= 768;
    const statsContainer = isMobile 
      ? document.getElementById('bookingsCalendarSlotSummary')
      : document.getElementById('calendarViewStats');
...
async function renderSimpleCustomerCalendar(customerBookings = []) {
  // On mobile, render to bookingsView container; on desktop, render to calendarView container
  const isMobile = window.innerWidth <= 768;
  const container = isMobile 
    ? document.getElementById('bookingsTeamCalendar')
