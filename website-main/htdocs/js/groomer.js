...
function switchGroomerViewMobile(view) {
  ...
  console.log('[Groomer Mobile] Available sections:', document.querySelectorAll('.groomer-content-section'));
  ...
  document.querySelectorAll('.groomer-content-section').forEach(section => {
  ...
  switch (view) {
    case 'overview':
      // Overview shows stats and recent bookings
      console.log('[Groomer Mobile] Showing overview (stats + recent bookings)');
      const overviewSection = document.getElementById('groomerOverviewSection');
      if (overviewSection) {
        overviewSection.classList.add('mobile-active');
    ...
    case 'profile':
      ...
      console.log('[Groomer Mobile] Profile section found:', profileSection);
      if (profileSection) {
        profileSection.classList.add('mobile-active');
      }
      break;
    case 'password':
      const passwordSection = document.getElementById('groomerPasswordSection');
      console.log('[Groomer Mobile] Password section found:', passwordSection);
      if (passwordSection) {
        passwordSection.classList.add('mobile-active');
      }
      break;
    case 'absence':
      const absenceSection = document.getElementById('groomerAbsenceSection');
      console.log('[Groomer Mobile] Absence section found:', absenceSection);
      if (absenceSection) {
        absenceSection.classList.add('mobile-active');
      }
      break;
    case 'calendar':
      const calendarSection = document.getElementById('groomerCalendarSection');
      console.log('[Groomer Mobile] Calendar section found:', calendarSection);
      if (calendarSection) {
        calendarSection.classList.add('mobile-active');
      }
      break;
    case 'history':
      const historySection = document.getElementById('groomerHistorySection');
      console.log('[Groomer Mobile] History section found:', historySection);
      if (historySection) {
    ...
    case 'bookings':
      ...
      console.log('[Groomer Mobile] Bookings section found:', bookingsSection);
      if (bookingsSection) {
...
window.loadGroomerOverview = loadGroomerOverview;
...
// ============================================
// 📊 GROOMER OVERVIEW (Mobile)
// ============================================

async function loadGroomerOverview() {
  try {
    const user = await getCurrentUser();
    if (!user || !groomerGroomerId) return;
    
    // Load stats in overview
    const allAbsences = getStaffAbsences();
    const myAbsences = allAbsences.filter(a => a.staffId === user.id);
    await renderGroomerOverviewStats(myAbsences);
    
    // Load recent bookings
    await renderGroomerRecentBookings();
  } catch (error) {
    console.warn('[Groomer Overview] Loading failed:', error);
  }
}

async function renderGroomerOverviewStats(absences) {
  const statsEl = document.getElementById('groomerOverviewStats');
  if (!statsEl) return;
  ...
  statsEl.innerHTML = `
    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; margin-bottom: 1.5rem;">
      <div class="stat-card" style="text-align: center; padding: 1rem; background: var(--white); border-radius: var(--radius); border: 1px solid #e5e7eb;">
        <div style="font-size: 1.5rem; margin-bottom: 0.5rem;">👥</div>
        <div class="stat-value" style="font-size: 1.5rem; font-weight: 700; color: var(--gray-900);">${customerCount}</div>
        <div class="stat-label" style="font-size: 0.8rem; color: var(--gray-600);">Customers</div>
      </div>
      <div class="stat-card" style="text-align: center; padding: 1rem; background: var(--white); border-radius: var(--radius); border: 1px solid #e5e7eb;">
        <div style="font-size: 1.5rem; margin-bottom: 0.5rem;">⏳</div>
        <div class="stat-value" style="font-size: 1.5rem; font-weight: 700; color: var(--gray-700);">${pending}</div>
        <div class="stat-label" style="font-size: 0.8rem; color: var(--gray-600);">Pending</div>
      </div>
    </div>
  `;
}

async function renderGroomerRecentBookings() {
  const container = document.getElementById('groomerRecentBookings');
  ...
  // Sort by creation date (newest first) and take only recent 3
  bookings.sort((a, b) => {
  ...
  if (recentBookings.length === 0) {
    container.innerHTML = `
      <div style="margin-bottom: 1rem;">
        <h4 style="margin-bottom: 0.75rem; font-size: 1rem; color: var(--gray-900);">📅 Recent Bookings</h4>
        <p class="empty-state" style="text-align: center; color: var(--gray-500); padding: 2rem;">No recent bookings assigned to you.</p>
      </div>
    `;
    return;
  }
  ...
  container.innerHTML = `
    <div style="margin-bottom: 1rem;">
      ...
      ${recentBookings.map(booking => {
        ...
        return `
          <div class="card" style="margin-bottom: 0.75rem; padding: 1rem; border: 1px solid #e5e7eb; cursor: pointer;" onclick="openGroomerBookingModal('${booking.id}')">
            <div style="display: flex; gap: 1rem; align-items: center;">
              <div style="font-size: 2rem;">${petEmoji}</div>
              <div style="flex: 1;">
                <h5 style="margin: 0 0 0.25rem 0; font-size: 0.95rem; color: var(--gray-900);">${escapeHtml(booking.petName)}</h5>
                <p style="margin: 0 0 0.25rem 0; font-size: 0.8rem; color: var(--gray-600);">${escapeHtml(booking.customerName || 'Walk-in')}</p>
                <p style="margin: 0; font-size: 0.75rem; color: var(--gray-500);">📅 ${formatDate(booking.date)} at ${formatTime(booking.time)}</p>
              </div>
              <div>
                <span class="badge ${statusClass}" style="font-size: 0.7rem;">${escapeHtml(statusLabel)}</span>
              </div>
            </div>
          </div>
        `;
      ...
      <div style="text-align: center; margin-top: 1rem;">
        <button class="btn btn-outline btn-sm" onclick="switchGroomerViewMobile('bookings')" style="width: auto; padding: 0.5rem 1rem;">View All Bookings</button>
      </div>
    </div>
  `;
}
...
async function loadGroomerCalendarLazy() {
  try {
    ...
    // Use simple customer-style calendar instead of renderMegaCalendar
    renderGroomerCalendar('groomerCalendar', bookings, myAbsences);
  } catch (error) {
...
// ============================================
// SIMPLE GROOMER CALENDAR - Customer Dashboard Style
// ============================================

// Simple groomer calendar based on customer calendar approach
function renderGroomerCalendar(containerId, bookings = [], absences = []) {
  const container = document.getElementById(containerId);
  if (!container) return;

  // Get current date
  const currentDate = new Date();
  const currentMonth = groomerCalendarMonth || currentDate.getMonth();
  const currentYear = groomerCalendarYear || currentDate.getFullYear();
  
  const monthNames = ['January', 'February', 'March', 'April', 'May', 'June',
    'July', 'August', 'September', 'October', 'November', 'December'];

  // Group bookings by date
  const groomerBookingsByDate = {};
  bookings.forEach(booking => {
    if (booking.date) {
      if (!groomerBookingsByDate[booking.date]) {
        groomerBookingsByDate[booking.date] = [];
      }
      groomerBookingsByDate[booking.date].push(booking);
    }
  });

  // Group absences by date
  const groomerAbsencesByDate = {};
  absences.forEach(absence => {
    if (absence.date) {
      if (!groomerAbsencesByDate[absence.date]) {
        groomerAbsencesByDate[absence.date] = [];
      }
      groomerAbsencesByDate[absence.date].push(absence);
    }
  });
  ...
  // EXACT customer calendar HTML structure
  let calendarHTML = `
    <div class="mega-calendar-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
      <button class="btn btn-outline btn-sm" onclick="changeGroomerCalendarMonth(-1)">← Prev</button>
      <h3 style="margin:0;">${monthNames[currentMonth]} ${currentYear}</h3>
      <button class="btn btn-outline btn-sm" onclick="changeGroomerCalendarMonth(1)">Next →</button>
    </div>
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const dayBookings = groomerBookingsByDate[dateStr] || [];
    const dayAbsences = groomerAbsencesByDate[dateStr] || [];
    const isToday = dateStr === todayISO;
    
    // Border color logic
    let borderColor = 'var(--gray-200)';
    if (dayAbsences.length > 0) borderColor = '#f44336'; // red for absence
    else if (dayBookings.length >= 3) borderColor = '#ff9800'; // busy day
    ...
    calendarHTML += `
      <div style="background:var(--white); border:1px solid ${borderColor}; border-radius:var(--radius-sm); padding:0.5rem; min-height:80px; cursor:pointer; position:relative; transition:all 0.2s;">
        ...
        <div style="margin-top:0.25rem;">
          ${dayAbsences.length > 0 ? `<div style="font-size:0.75rem; color:#f44336; font-weight:600;">Absent</div>` : ''}
          ${dayBookings.slice(0, 3).map(booking => {
            ...
            return `<div style="font-size:0.75rem; color:var(--gray-700); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; cursor:pointer;" title="${booking.petName} - ${booking.packageName || 'Custom'} (${booking.status})">
...
// Calendar month navigation for groomer
let groomerCalendarMonth = new Date().getMonth();
let groomerCalendarYear = new Date().getFullYear();

function changeGroomerCalendarMonth(direction) {
  groomerCalendarMonth += direction;
  if (groomerCalendarMonth < 0) {
    groomerCalendarMonth = 11;
    groomerCalendarYear--;
  } else if (groomerCalendarMonth > 11) {
    groomerCalendarMonth = 0;
    groomerCalendarYear++;
  }
...
window.changeGroomerCalendarMonth = changeGroomerCalendarMonth;
window.renderGroomerCalendar = renderGroomerCalendar;
// ============================================
// SIDEBAR NAVIGATION
// ============================================

function switchGroomerView(view) {
  ...
  const targetSection = document.getElementById(`groomer${view.charAt(0).toUpperCase() + view.slice(1)}Section`);
  if (targetSection) {
...
document.addEventListener('DOMContentLoaded', function() {
  document.querySelectorAll('.sidebar-menu a').forEach(link => {
    link.addEventListener('click', function(e) {
      ...
      switchGroomerView(view);
    });
  });
});

// Make function global
window.switchGroomerView = switchGroomerView;