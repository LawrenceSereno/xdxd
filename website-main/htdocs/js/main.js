...
function canSubmit(userId) {
  ...
  if (current.status === 'submitting') {
    // Check if stale
    const age = Date.now() - (current.startedAt || 0);
    // Allow if older than 5 seconds (likely stuck) or 30 seconds (definitely stale)
    if (age >= 5000) {
      console.log('[canSubmit] Submission is stuck (age:', Math.round(age / 1000), 'seconds), allowing new submission');
      return true;
    }
    return false; // Active submission (less than 5 seconds)
  }
