...
async function initBooking() {
  ...
  // ============================================
  // 🧹 CLEAR STALE SUBMISSION STATUS
  // Always reset submission status on page load to prevent stuck submissions
  // This ensures fresh submissions can proceed
  // ============================================
  if (typeof resetSubmissionStatus === 'function') {
    const status = typeof getSubmissionStatus === 'function' ? getSubmissionStatus() : null;
    if (status && status.status !== 'idle') {
      const age = Date.now() - (status.startedAt || 0);
      console.log('[initBooking] 🧹 Clearing submission status (was:', status.status, 'age:', Math.round(age / 1000), 'seconds)');
  ...
  // NOTE: Do NOT clear lastBooking here - it interferes with the redirect flow
  // The booking-success page already handles clearing old bookings (checks age)
  // Clearing here causes issues when using "Use Saved Details" flow
  // ============================================
  // sessionStorage.removeItem('lastBooking'); // REMOVED - causes redirect issues
  console.log('[initBooking] Starting new booking (lastBooking will be set after submission)');
    ...
    if (user) {
      try {
        ...
        if (autoLoad === 'true' && !isEditingExistingBooking) {
          ...
          // 🔧 CRITICAL: Clear any editing flags when using "Use Saved Details"
          // This ensures we create a NEW booking, not edit an existing one
          sessionStorage.removeItem('editingBookingId');
          delete bookingData.editingBookingId;
          
          // 🔧 CRITICAL: Clear old lastBooking to prevent stale payment data
          // This fixes the bug where old paymentChoice was being shown on booking-success
          sessionStorage.removeItem('lastBooking');
          console.log('[initBooking] Cleared old lastBooking to prevent stale payment data');

          // 🔴 CRITICAL: Mark that we used autoLoad so submitBooking knows to create NEW booking
          bookingData.wasAutoLoaded = true;
          sessionStorage.setItem('wasAutoLoaded', 'true');

          console.log('[initBooking] Use Saved Details: Cleared editing flags to ensure new booking creation');
          console.log('[initBooking] Set wasAutoLoaded flag to force NEW booking mode');
...
function setupBookingListeners() {
  ...
  // Add global function for debugging submission status
  window.debugSubmissionStatus = function () {
    console.log('=== SUBMISSION STATUS DEBUG ===');
    console.log('isSubmittingBooking:', isSubmittingBooking);
    console.log('lastSubmitTime:', lastSubmitTime);
    console.log('SUBMIT_COOLDOWN_MS:', SUBMIT_COOLDOWN_MS);
    console.log('Time since last submit:', Date.now() - lastSubmitTime);
    if (typeof getSubmissionStatus === 'function') {
      console.log('Database submission status:', getSubmissionStatus());
    }
    if (typeof canSubmit === 'function') {
      console.log('canSubmit() result:', canSubmit('test-user'));
    }
    console.log('bookingData.wasAutoLoaded:', bookingData.wasAutoLoaded);
    console.log('sessionStorage.wasAutoLoaded:', sessionStorage.getItem('wasAutoLoaded'));
    console.log('sessionStorage.autoLoadProfile:', sessionStorage.getItem('autoLoadProfile'));
    console.log('sessionStorage.editingBookingId:', sessionStorage.getItem('editingBookingId'));
  };

  // Add global function to force reset submission status (for debugging stuck submissions)
  window.forceResetSubmission = function () {
    console.log('[DEBUG] Force resetting submission status...');
    isSubmittingBooking = false;
    lastSubmitTime = 0;
    if (typeof resetSubmissionStatus === 'function') {
      resetSubmissionStatus();
      console.log('[DEBUG] ✅ Submission status reset');
    }
    // Also clear the wasAutoLoaded flag if needed
    sessionStorage.removeItem('wasAutoLoaded');
    delete bookingData.wasAutoLoaded;
    console.log('[DEBUG] Cleared wasAutoLoaded flags');
    window.debugSubmissionStatus();
  };
}
...
async function handleProfileLoad() {
  ...
  try {
    ...
    const bookingsTimeout = new Promise((_, reject) => {
      setTimeout(() => reject(new Error('Bookings fetch timeout')), 15000);
    });
...
async function submitBooking(event) {
  ...
  console.log('[submitBooking] 🚀 SUBMIT BUTTON CLICKED - Starting submission...');
  console.log('[submitBooking] ========================================');
  console.log('[submitBooking] Event:', event);
  console.log('[submitBooking] Current step:', currentStep);
  console.log('[submitBooking] bookingData.wasAutoLoaded:', bookingData.wasAutoLoaded);
  console.log('[submitBooking] sessionStorage.wasAutoLoaded:', sessionStorage.getItem('wasAutoLoaded'));

  // Get user info for logging
  const user = await getCurrentUser().catch(() => null);
  const userId = user?.id || 'anonymous';

  console.log('[submitBooking] User:', userId);
  ...
  if (isSubmittingBooking) {
    ...
    console.log('[submitBooking] isSubmittingBooking:', isSubmittingBooking);
    console.log('[submitBooking] EARLY RETURN - Not proceeding with submission');
  ...
  console.log('[submitBooking] ✅ CHECK 1 PASSED - Local flag check OK');
  ...
  if (typeof canSubmit === 'function' && !canSubmit(userId)) {
    ...
    console.log('[submitBooking] canSubmit() returned false');
    console.log('[submitBooking] EARLY RETURN - Not proceeding with submission');
    console.log('[submitBooking] 🔧 WORKAROUND: Forcing submission anyway to bypass stuck status');
    
    // TEMPORARY WORKAROUND: Force reset and continue
    if (typeof resetSubmissionStatus === 'function') {
      resetSubmissionStatus();
      console.log('[submitBooking] 🧹 Reset submission status and continuing...');
    }
  } else {
    console.log('[submitBooking] ✅ CHECK 2 PASSED - Database status check OK');
  }
  ...
  if (timeSinceLastSubmit < SUBMIT_COOLDOWN_MS) {
    ...
    console.log('[submitBooking] timeSinceLastSubmit:', timeSinceLastSubmit, 'SUBMIT_COOLDOWN_MS:', SUBMIT_COOLDOWN_MS);
    console.log('[submitBooking] EARLY RETURN - Not proceeding with submission');
  ...
  console.log('[submitBooking] ✅ CHECK 3 PASSED - Cooldown check OK');
  ...
  if (typeof startSubmission === 'function') {
    ...
    console.log('[submitBooking] startSubmission() returned:', submissionToken);
  ...
  console.log('[submitBooking] ✅ Submission lock acquired, token:', submissionToken);
  ...
  try {
    ...
    // ============================================
    // 🛡️ PET NAME CHANGE VALIDATION
    // Prevent abuse: If duplicate was detected, ensure pet name actually changed
    // ============================================
    const duplicateDetectedAt = sessionStorage.getItem('duplicateDetectedAt');
    if (duplicateDetectedAt) {
      const duplicateOriginalName = sessionStorage.getItem('duplicateOriginalPetName');
      const duplicateOriginalDate = sessionStorage.getItem('duplicateOriginalDate');
      const duplicateOriginalTime = sessionStorage.getItem('duplicateOriginalTime');

      // Check if pet name is STILL the same as the duplicate
      if (bookingData.petName === duplicateOriginalName &&
        bookingData.date === duplicateOriginalDate &&
        bookingData.time === duplicateOriginalTime) {

        console.warn('[submitBooking] BLOCKED - Pet name not changed after duplicate warning');

        if (typeof hideLoadingOverlay === 'function') hideLoadingOverlay();

        await customAlert.warning(
          'Pet Name Not Changed',
          `You must change the pet name from "${duplicateOriginalName}" to submit.\n\n` +
      ...
      // Pet name was changed - clear the duplicate flags
      sessionStorage.removeItem('duplicateDetectedAt');
      sessionStorage.removeItem('duplicateOriginalPetName');
      sessionStorage.removeItem('duplicateOriginalDate');
      sessionStorage.removeItem('duplicateOriginalTime');
      console.log('[submitBooking] Pet name changed successfully - duplicate flags cleared');
    }
    ...
    console.log('[submitBooking] Checking user warnings...', { isAdminWalkIn, isAdmin, userId: user.id });

    // Only check warnings for regular customers (not admin walk-in)
    if (!isAdminWalkIn && !isAdmin) {
      console.log('[submitBooking] Checking warnings for regular customer...');
      const warningInfo = await getCustomerWarningInfo(user.id);
      console.log('[submitBooking] Warning info:', warningInfo);
      if (warningInfo?.isBanned) {
        console.log('[submitBooking] User is banned, redirecting to dashboard');
        if (typeof hideLoadingOverlay === 'function') hideLoadingOverlay();
        if (submitBtn) {
          submitBtn.disabled = false;
          submitBtn.textContent = 'Submit Booking';
        ...
        console.log('[submitBooking] User is not banned - proceeding with booking submission');
      }
    } else {
      console.log('[submitBooking] Skipping warning check for admin walk-in or admin user');
    }
    ...
    console.log('[submitBooking] DEBUGGING - Checking editing mode:');
    console.log('[submitBooking] bookingData.editingBookingId:', bookingData.editingBookingId);
    console.log('[submitBooking] sessionStorage editingBookingId:', sessionStorage.getItem('editingBookingId'));
    console.log('[submitBooking] Final editingId:', editingId);

    // 🔴 CRITICAL FIX: If autoLoadProfile was used, NEVER treat as editing
    // autoLoadProfile means "Use Saved Details" which should create a NEW booking
    // NOTE: autoLoadProfile flag is cleared in initBooking, so we rely on bookingData.wasAutoLoaded
    const wasAutoLoaded = bookingData.wasAutoLoaded === true;
    console.log('[submitBooking] Checking wasAutoLoaded:', {
      bookingData_wasAutoLoaded: bookingData.wasAutoLoaded,
      sessionStorage_wasAutoLoaded: sessionStorage.getItem('wasAutoLoaded'),
      finalResult: wasAutoLoaded
    });

    if (wasAutoLoaded) {
      console.log('[submitBooking] ⚠️ AUTO-LOADED profile detected - FORCING new booking mode (ignoring editingId)');
      // Clear any editing flags
      sessionStorage.removeItem('editingBookingId');
      delete bookingData.editingBookingId;
    }

    if (editingId && !wasAutoLoaded) {
      console.log('[submitBooking] ⚠️ EDITING MODE DETECTED - This will redirect to booking-success via editing flow');
      ...
      // Redirect immediately to booking-success.html
      window.location.href = 'booking-success.html';
      return;
    }

    // Existing create flow for new booking
    console.log('[submitBooking] ✅ NEW BOOKING MODE - Creating fresh booking');
    console.log('[submitBooking] This will redirect to booking-success via main flow');
    ...
    if (isDuplicate) {
      console.warn('[submitBooking] Duplicate booking detected - asking user to change pet name');

      // ============================================
      // 🔄 DUPLICATE DETECTED - Let customer change name
      // ============================================
      ...
      // Generate pet name suggestions for reference
      const originalName = booking.petName;
      const suggestions = [
        `${originalName} 2`,
        `${originalName} (2nd)`,
        `${originalName} Jr.`,
      ...
      // Show warning with suggestions - customer must manually change the name
      await customAlert.warning(
        'Pet Name Already Booked',
        `You already have a booking for "${originalName}" on ${booking.date} at ${booking.time}.\n\n` +
        `Please change the pet name to something different.\n\n` +
        `Suggestions:\n• ${suggestions.join('\n• ')}\n\n` +
        `Or use any other name you prefer.`
      );

      // Reset submission flag
      isSubmittingBooking = false;

      // Go back to pet info step (step 2) - customer manually changes the name
      showStep(2);

      // ============================================
      // 🛡️ ANTI-ABUSE PROTECTION
      // ============================================
      // Set a flag that duplicate was detected
      // This prevents rapid re-submission attempts
      sessionStorage.setItem('duplicateDetectedAt', Date.now().toString());
      sessionStorage.setItem('duplicateOriginalPetName', originalName);
      sessionStorage.setItem('duplicateOriginalDate', booking.date);
      sessionStorage.setItem('duplicateOriginalTime', booking.time);

      // Disable submit button temporarily (3 seconds)
      ...
      if (submitBtn) {
        ...
        setTimeout(() => {
          enableSubmitButton();
          console.log('[submitBooking] Submit button re-enabled after duplicate warning (3s cooldown)');
        }, 3000);
      }
    ...
    console.log('[submitBooking] Booking history logged successfully');
    ...
    // ============================================
    // 🔴 CRITICAL: SAVE BOOKING TO SESSIONSTORAGE
    // ============================================
    console.log('[submitBooking] 🔄 Saving booking to sessionStorage...');
    console.log('[submitBooking] 🔍 Booking paymentChoice before save:', booking.paymentChoice);

    // Validate booking object before saving
    if (!booking || !booking.id) {
      console.error('[submitBooking] 🚨 CRITICAL: Invalid booking object!', booking);
      throw new Error('Invalid booking object - cannot save to sessionStorage');
    }

    // 🔧 CRITICAL FIX: Ensure paymentChoice is null for new bookings
    // This prevents old paymentChoice values from being carried over
    if (booking.paymentChoice !== null) {
      console.warn('[submitBooking] ⚠️ paymentChoice was not null, forcing to null:', booking.paymentChoice);
      booking.paymentChoice = null;
    }
    ...
    try {
      ...
      console.log('[submitBooking] 🔍 Final booking paymentChoice:', booking.paymentChoice);

      // CRITICAL: Clear old data AFTER preparing new data (not before)
      // This prevents a race condition where the page redirects before new data is saved
      sessionStorage.removeItem('lastBooking');

      // Save the new booking
      sessionStorage.setItem('lastBooking', bookingJSON);
      
      // Also save a debug flag to track the redirect
      sessionStorage.setItem('_bookingRedirectDebug', JSON.stringify({
        savedAt: Date.now(),
        bookingId: booking.id,
      ...
      console.log('[submitBooking] ✅ Booking saved to sessionStorage successfully');

      // VERIFY it was actually saved
      const verification = sessionStorage.getItem('lastBooking');
      if (!verification) {
        console.error('[submitBooking] 🚨 CRITICAL: Booking NOT found in sessionStorage after save!');
        throw new Error('Failed to save booking to sessionStorage');
      }

      const verifiedBooking = JSON.parse(verification);
      console.log('[submitBooking] ✅ Verified booking in sessionStorage:', {
        id: verifiedBooking.id,
        petName: verifiedBooking.petName,
        _savedAt: verifiedBooking._savedAt
      });

      // CRITICAL: Also clear other session data that might interfere
      sessionStorage.removeItem('bookingData');
      sessionStorage.removeItem('bookingStep');
      sessionStorage.removeItem('autoLoadProfile');
      sessionStorage.removeItem('wasAutoLoaded');
      console.log('[submitBooking] ✅ Cleared old booking session data');

    } catch (error) {
      console.error('[submitBooking] 🚨 CRITICAL ERROR saving to sessionStorage:', error);
      throw new Error('Cannot save booking to sessionStorage: ' + error.message);
    }
    ...
    console.log('[submitBooking] About to redirect to booking-success.html');
    console.log('[submitBooking] Current URL:', window.location.href);
    console.log('[submitBooking] Booking saved to sessionStorage:', {
      id: booking.id,
      petName: booking.petName,
      date: booking.date,
      time: booking.time
    ...
    // ============================================
    // 🔴 CRITICAL: IMMEDIATE REDIRECT TO BOOKING SUCCESS
    // ============================================
    console.log('[submitBooking] 🚀 REDIRECTING to booking-success.html');

    // VERIFY booking is in sessionStorage RIGHT before redirect
    const verifyBooking = sessionStorage.getItem('lastBooking');
    console.log('[submitBooking] SessionStorage check before redirect:', verifyBooking ? 'EXISTS' : 'MISSING');
    if (!verifyBooking) {
      console.error('[submitBooking] 🚨 CRITICAL: No lastBooking in sessionStorage! This will cause redirect to dashboard!');
    }

    // Redirect IMMEDIATELY without delay to prevent race conditions
    console.log('[submitBooking] Final redirect to booking-success.html');
    console.log('[submitBooking] SessionStorage lastBooking exists:', !!sessionStorage.getItem('lastBooking'));
