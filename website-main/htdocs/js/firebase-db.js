...
const USERS_CACHE_DURATION = 30000; // 30 seconds cache (reduced from 60s for faster updates)
let usersPermissionDenied = false; // Track if permission was denied

// Force refresh users cache (bypass cache duration)
function invalidateUsersCache() {
  usersCache = null;
  usersCacheTime = 0;
}

// Validate and clean up users - remove duplicates and orphaned records
async function validateAndCleanupUsers() {
  try {
    const db = getDatabase();
    if (!db) {
    ...
    // Track duplicates and valid users
    const emailMap = {};
    const usersToKeep = {};
    const duplicates = [];

    // First pass: identify duplicates by email
    userIds.forEach(id => {
      ...
      if (email) {
        if (emailMap[email]) {
          // Duplicate found - keep the one with more recent createdAt
          const existing = emailMap[email];
          const existingUser = usersData[existing.id];
          
          if ((user.createdAt || 0) > (existingUser.createdAt || 0)) {
            // New user is more recent, remove old one
            duplicates.push(existing.id);
            emailMap[email] = { id, user };
          } else {
            // Existing user is more recent, remove new one
            duplicates.push(id);
          }
        } else {
          emailMap[email] = { id, user };
    ...
    if (duplicates.length > 0) {
      console.log('Removing duplicate users:', duplicates);
      for (const dupId of duplicates) {
        const dupRef = ref(db, `users/${dupId}`);
        await remove(dupRef);
      }
      ...
      console.log(`✓ Removed ${duplicates.length} duplicate user records`);
    } else {
...
window.validateAndCleanupUsers = validateAndCleanupUsers;
...
async function getUsers(forceRefresh = false) {
  ...
  // Return cached data if still valid (unless forceRefresh is true)
  if (!forceRefresh && usersCache && (Date.now() - usersCacheTime) < USERS_CACHE_DURATION) {
    return usersCache;
  }
...
async function createBooking(booking) {
  try {
    ...
    if (booking.paymentChoice !== null) {
      console.warn('[createBooking] ⚠️ Forcing paymentChoice to null');
